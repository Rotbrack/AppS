...
apps.get("/login", async (req, res) => {
    ...
    console.log("Generiere Auth URL:", url);
    res.redirect(url);
});
...
apps.get("/auth/callback", async (req, res) => {
    if (!req.query.code) {
        console.error("Callback ohne code:", req.query);
        return res.status(400).send("Auth-Code fehlt");
    }

    const codeVerifier = req.session.pkceCodeVerifier;
    if (!codeVerifier) {
        console.error("PKCE Verifier fehlt in Session");
        return res.status(400).send("PKCE Verifier fehlt");
    }
...
apps.get("/auth/callback/admindash", async (req, res) => {
    if (!req.query.code) {
        console.error("Admin-Callback ohne code:", req.query);
        return res.status(400).send("Auth-Code fehlt");
    }
